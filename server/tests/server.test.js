...
describe('Delete /todos/:id', () => {
	it('should remove todo', (done) => {
		...
		request(app)
			.delete(`/todos/${hexId}`)
			.set('x-auth',users[1].token[0].token)
			.expect(200)
			.expect((res) => {
...
describe('PATCH /todos/:id',() => {
	...
	it('should clear completedAt when todo is not completed', (done) => { 
		var hexId = todos[1]._id.toHexString();
		var text = "this is the diff updated"; 
		request(app) 
		.patch(`/todos/${hexId}`)
		.set('x-auth',users[1].token[0].token)
		.send({
		...
		request(app)
			...
			.end((err,res) =>{
				...
				Users.findById(users[1]._id).then((users) => {
					expect(users.token[1]).toMatchObject({
						access: 'auth',
	...
	it('should reject invalid login',(done) => {
		request(app)
			...
			.end((err,res) =>{
				...
				Users.findById(users[1]._id).then((users) => {
					expect(users.token.length).toBe(1);
					done();
				}).catch((err) => done(err));
			});
	})
